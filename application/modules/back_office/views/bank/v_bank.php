                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    ...
                                    <h4 class="page-title">Data Master Bank</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table id="bank_datatable" class="table dt-responsive nowrap">
                                            <thead>
                                                <tr>
                                                    <th>No</th>
                                                    <th>Nama Bank</th>
                                                    <th>Nomor Rekekning</th>
...
$(document).ready(function () {
    var urll = "<?php echo base_url('back_office/bank/get_all_bank'); ?>";
    var tabel = $('#bank_datatable').DataTable({
        ajax: {
            "url": urll,
            "type": "POST"
        },
    ...
    $('#bank_datatable tbody').on('click', '.edit', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        simpan = "update";
        $('#modal').load("<?php echo base_url()?>back_office/bank/modal_update_bank/"+data['bank_id']);
    });

    $('#bank_datatable tbody').on('click', '.hapus', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url()?>back_office/bank/delete_bank/" + data['bank_id'],
                    type: "POST",
                    dataType: "JSON",
                    success: function (b) {
                        tabel.ajax.reload();
...
function tambah() {
    simpan = "create";
    $('#modal').load("<?php echo base_url("back_office/bank/modalbank")?>");
}
</script>

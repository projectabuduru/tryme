...
<html lang="en">
    <head>
        ...
        <!-- Plugins css-->
        <link href="<?php echo base_url('assets/bo/libs/bootstrap-datepicker/bootstrap-datepicker.min.css');?>" rel="stylesheet" type="text/css" />
        <link href="<?php echo base_url('assets/bo/libs/daterangepicker/daterangepicker.css');?>" rel="stylesheet" type="text/css" />
    ...
    <body>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table id="pengeluaran_datatable" class="table dt-responsive nowrap">
                                            <thead>
                                                <tr>
                                                    <th>No</th>
                                                    <th>Nama</th>
        ...
        <script src="<?php echo base_url('assets/bo/libs/bootstrap-datepicker/bootstrap-datepicker.min.js');?>"></script>
        <script src="<?php echo base_url('assets/bo/libs/daterangepicker/daterangepicker.js');?>"></script>
...
$(document).ready(function () {
    var urll = "<?php echo base_url('back_office/pengeluaran/get_all_pengeluaran'); ?>";
    var tabel = $('#pengeluaran_datatable').DataTable({
        ajax: {
            "url": urll,
            "type": "POST"
        },
        ...
        columns: [
            ...
            {data: "tgl_pengeluaran"},
            {
                "mRender": function (data, type, row) {
                    var tampil = "";
                    tampil += ' <button title="Edit Pengeluaran" class="edit btn btn-info fas fa-pencil-alt"><i class=""></i></button>'; 
                    tampil += ' <button title="Hapus Pengeluaran" class="hapus btn btn btn-danger fas fa-trash-alt"><i class=""></button>';
                    return tampil;
                },
    ...
    $('#pengeluaran_datatable tbody').on('click', '.edit', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        simpan = "update";
        $('#modal').load("<?php echo base_url()?>back_office/pengeluaran/modal_update_pengeluaran/"+data['id_pengeluaran']);
    });

    $('#pengeluaran_datatable tbody').on('click', '.hapus', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url()?>back_office/pengeluaran/delete_pengeluaran/" + data['id_pengeluaran'],
                    type: "POST",
                    dataType: "JSON",
                    success: function (b) {
                        tabel.ajax.reload();
...
function tambah() {
    simpan = "create";
    $('#modal').load("<?php echo base_url("back_office/pengeluaran/modalpengeluaran")?>");
}
</script>

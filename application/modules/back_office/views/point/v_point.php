                            ...
                            <div class="page-title-box">
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        ...
                                        <li class="breadcrumb-item active">Point Reward</li>
                                    </ol>
                                </div>
                                <h4 class="page-title">Point Reward</h4>
                            </div>
                        </div>
                    </div>
                    <!-- end page title -->
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <table id="point_datatable" class="table dt-responsive nowrap">
                                        <thead>
                                            <tr>
                                                <th>No</th>
                                                <th>Minimal Transaksi</th>
                                                <th>Maximal Transaksi</th>
                                                <th>Jumnlah Point</th>
                                                <th>Status</th>
                                                <th>Action</th>
                                            </tr>
...
<script type="text/javascript">
    $(document).ready(function() {
        var urll = "<?php echo base_url('back_office/point/get_all_point'); ?>";
        var tabel = $('#point_datatable').DataTable({
            ajax: {
                "url": urll,
                "type": "POST"
            },
            ...
            columns: [{
                ...
                },
                {
                    "mRender": function(data, type, row) {
                        var tampil = "";
                        tampil += ' <button title="Edit Point" class="edit btn btn-info fas fa-pencil-alt"><i class=""></i></button>';
                        tampil += ' <button title="Hapus Point" class="hapus btn btn btn-danger fas fa-trash-alt"><i class=""></button>';
                        return tampil;
                    },
        ...
        $('#point_datatable tbody').on('click', '.edit', function(e) {
            var data = tabel.row($(this).parents('tr')).data();
            simpan = "update";
            $('#modal').load("<?php echo base_url() ?>back_office/point/modal_update_point/" + data['point_id']);
        });

        $('#point_datatable tbody').on('click', '.hapus', function(e) {
            var data = tabel.row($(this).parents('tr')).data();
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "<?php echo base_url() ?>back_office/point/delete_point/" + data['point_id'],
                        type: "POST",
                        dataType: "JSON",
                        success: function(b) {
                            tabel.ajax.reload();
    ...
    function tambah() {
        simpan = "create";
        $('#modal').load("<?php echo base_url("back_office/point/modalpoint") ?>");
    }
</script>
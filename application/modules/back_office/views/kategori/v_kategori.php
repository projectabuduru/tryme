                                ...
                                <div class="page-title-box">
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            ...
                                            <li class="breadcrumb-item active">Data Kategori Product</li>
                                        </ol>
                                    </div>
                                    <h4 class="page-title">Data Master Kategori Produk</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table id="kategori_datatable" class="table dt-responsive nowrap">
                                            <thead>
                                                <tr>
                                                    <th>No</th>
                                                    <th>Nama Kategori</th>
...
$(document).ready(function () {
    var urll = "<?php echo base_url('back_office/kategori/get_all_kategori'); ?>";
    var tabel = $('#kategori_datatable').DataTable({
        ajax: {
            "url": urll,
            "type": "POST"
        },
        ...
        columns: [
            ...
            {data: "status"},
            {
                "mRender": function (data, type, row) {
                    var tampil = "";
                    tampil += ' <button title="Edit Kategori" class="edit btn btn-info fas fa-pencil-alt"><i class=""></i></button>'; 
                    tampil += ' <button title="Hapus Kategori" class="hapus btn btn btn-danger fas fa-trash-alt"><i class=""></button>';
                    return tampil;
                },
    ...
    $('#kategori_datatable tbody').on('click', '.edit', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        simpan = "update";
        $('#modal').load("<?php echo base_url()?>back_office/kategori/modal_update_kategori/"+data['cate_id']);
    });

    $('#kategori_datatable tbody').on('click', '.hapus', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url()?>back_office/kategori/delete_kategori/" + data['cate_id'],
                    type: "POST",
                    dataType: "JSON",
                    success: function (b) {
                        tabel.ajax.reload();
...
function tambah() {
    simpan = "create";
    $('#modal').load("<?php echo base_url("back_office/kategori/modalkategori")?>");
}
</script>

                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    ...
                                    <h4 class="page-title">Data Master Admin</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table id="admin_datatable" class="table dt-responsive nowrap">
                                            <thead>
                                                <tr>
                                                    <th>No</th>
                                                    <th>Nama Admin</th>
...
$(document).ready(function () {
    var urll = "<?php echo base_url('back_office/admin/get_all_admin'); ?>";
    var tabel = $('#admin_datatable').DataTable({
        ajax: {
            "url": urll,
            "type": "POST"
        },
        ...
        columns: [
            ...
            {data: "status"},
            {
                "mRender": function (data, type, row) {
                    var tampil = "";
                    tampil += ' <button title="Edit Admin" class="edit btn btn-info fas fa-pencil-alt"><i class=""></i></button>'; 
                    tampil += ' <button title="Hapus Admin" class="hapus btn btn btn-danger fas fa-trash-alt"><i class=""></button>';
                    return tampil;
                },
    ...
    $('#admin_datatable tbody').on('click', '.edit', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        simpan = "update";
        $('#modal').load("<?php echo base_url()?>back_office/admin/modal_update_admin/"+data['user_id']);
    });

    $('#admin_datatable tbody').on('click', '.hapus', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url()?>back_office/admin/delete_admin/" + data['user_id'],
                    type: "POST",
                    dataType: "JSON",
                    success: function (b) {
                        tabel.ajax.reload();
...
function tambah() {
    simpan = "create";
    $('#modal').load("<?php echo base_url("back_office/admin/modaladmin")?>");
}
</script>

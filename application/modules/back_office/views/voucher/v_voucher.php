                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table id="voucher_datatable" class="table dt-responsive nowrap">
                                            <thead>
                                                <tr>
                                                    <th>No</th>
                                                    <th>Nama Voucher</th>
...
$(document).ready(function () {
    var urll = "<?php echo base_url('back_office/voucher/get_all_voucher'); ?>";
    var tabel = $('#voucher_datatable').DataTable({
        ajax: {
            "url": urll,
            "type": "POST"
        },
        ...
        columns: [
            ...
            {data: "expired"},
            {
                "mRender": function (data, type, row) {
                    var tampil = "";
                    tampil += ' <button title="Edit Voucher" class="edit btn btn-info fas fa-pencil-alt"><i class=""></i></button>'; 
                    tampil += ' <button title="Hapus Voucher" class="hapus btn btn btn-danger fas fa-trash-alt"><i class=""></button>';
                    return tampil;
                },
    ...
    $('#voucher_datatable tbody').on('click', '.edit', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        simpan = "update";
        $('#modal').load("<?php echo base_url()?>back_office/voucher/modal_update_voucher/"+data['voc_id']);
    });

    $('#voucher_datatable tbody').on('click', '.hapus', function(e){
        var data = tabel.row( $(this).parents('tr') ).data();
        swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url()?>back_office/voucher/delete_voucher/" + data['voc_id'],
                    type: "POST",
                    dataType: "JSON",
                    success: function (b) {
                        tabel.ajax.reload();
...
function tambah() {
    simpan = "create";
    $('#modal').load("<?php echo base_url("back_office/voucher/modalvoucher")?>");
}
</script>
